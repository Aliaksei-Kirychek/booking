...
async def test_booking_crud(db: DBManager):
    ...
    booking_data.date_to = date(year=2025, month=3, day=24)
    modified_booking = await db.bookings.edit(booking_data, id=new_booking.id)
    assert len(modified_booking) == 1
    assert modified_booking[0].date_to == date(year=2025, month=3, day=24)
